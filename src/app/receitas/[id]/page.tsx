...
export default function EditRecipePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [ingRes, recRes, recipeRes] = await Promise.all([
          fetch('/api/ingredientes'),
          fetch('/api/receitas'),
          fetch(`/api/receitas/${id}`),
        ])

        const allIngredients = ingRes.ok ? await ingRes.json() : []
        const allRecipes = recRes.ok ? await recRes.json() : []

        setIngredients(allIngredients)
        setRecipes(allRecipes.filter((r: Recipe) => r.id !== Number(id)))
        ...
        if (recipeRes.ok) {
          ...
          const loadedItems: RecipeItem[] = recipe.items?.map((item: {
            ingredientId?: number
            subRecipeId?: number
            quantity: number
            ...
            subRecipeTotalCost?: number
            subRecipeYieldQuantity?: number
          }) => {
            if (item.ingredientId) {
              const ing = allIngredients.find((i: Ingredient) => i.id === item.ingredientId)
              return {
                type: 'ingredient' as const,
                ingredientId: item.ingredientId,
                quantity: String(item.quantity),
                label: `${item.ingredientName || ing?.name || 'Ingrediente'} (${item.ingredientUnit || ing?.unit || ''})`,
                costPerUnit: item.ingredientCostPerUnit ?? ing?.costPerUnit ?? 0,
              }
            } else {
              return {
                ...
                quantity: String(item.quantity),
                label: `[Sub] ${item.subRecipeName || 'Sub-receita'}`,
                costPerUnit: item.subRecipeTotalCost && item.subRecipeYieldQuantity
                  ? item.subRecipeTotalCost / item.subRecipeYieldQuantity
                  : 0,
                yieldQuantity: item.subRecipeYieldQuantity,
              }
            }
          }) ?? []
          ...
          toast({ title: 'Receita não encontrada', variant: 'destructive' })
  ...
  const totalCost = items.reduce((sum, item) => sum + item.costPerUnit * Number(item.quantity), 0)
